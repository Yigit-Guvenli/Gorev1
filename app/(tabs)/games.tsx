...
const GameCard: React.FC<{ game: Game; index: number; onPress: () => void }> = ({ game, index, onPress }) => {
  ...
  return (
    <Animated.View style={[styles.gameCard, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
      <TouchableOpacity style={styles.gameCardInner} activeOpacity={0.8} onPress={onPress}>
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={{ flexDirection: "row", flexWrap: "wrap", paddingTop: 12, gap: 8 }}>
          {CATEGORIES.map((cat) => (
            <CategoryButton key={cat} label={cat} active={activeCategory === cat} onPress={() => setActiveCategory(cat)} />
          ))}
        </View>
      </Animated.View>
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    backgroundColor: "#fdfaf2", paddingHorizontal: 24,
    paddingTop: 64, paddingBottom: 14,
    borderBottomWidth: 1, borderBottomColor: "#ede8d8",
  },
  ...
  resultCount: { fontSize: 13, color: "#9ca3af", marginBottom: 12, fontWeight: "600" },
  gameCard: {
    backgroundColor: "#f5f0e0", borderRadius: 16,
    borderWidth: 1, borderColor: "#e8e0c8",
    marginBottom: 12, overflow: "hidden",
  },
  gameCardInner: { flexDirection: "row", alignItems: "center", padding: 16, gap: 14 },
  gameEmoji: {
    width: 56, height: 56, borderRadius: 14,
    backgroundColor: "#fdfaf2", alignItems: "center", justifyContent: "center",
    borderWidth: 1, borderColor: "#e8e0c8",
  },
  gameEmojiText: { fontSize: 28 },
  gameInfo: { flex: 1 },
  gameHeader: { flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 4, flexWrap: "wrap" },
  gameTitle: { fontSize: 15, fontWeight: "800", color: "#111827" },
  gameCategoryBadge: { backgroundColor: "#e5ae32", paddingHorizontal: 8, paddingVertical: 2, borderRadius: 6 },
  gameCategoryText: { fontSize: 10, fontWeight: "800", color: "#111827" },
  gameDesc: { fontSize: 12, color: "#6b7280", lineHeight: 18 },
  playBtn: {
    width: 40, height: 40, borderRadius: 20,
    backgroundColor: "#e5ae32", alignItems: "center", justifyContent: "center",
  },
